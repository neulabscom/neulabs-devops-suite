...
def aws_assume_role(role, account_id, profile=None, token_code=None, serial_number=None):
    ...
    try:
        ...
        add_export_to_activate_file("AWS_ACCESS_KEY_ID", response["Credentials"]["AccessKeyId"])
        add_export_to_activate_file("AWS_SESSION_TOKEN", response["Credentials"]["SessionToken"])
        add_export_to_activate_file("AWS_SECRET_ACCESS_KEY", response["Credentials"]["SecretAccessKey"])
...
def default_env_vars(company, env, config_path):    
    ...
    add_export_to_activate_file("CONFIG_TENANT_SETTINGS_PATH", str(config_path))
    add_export_to_activate_file("KUBECONFIG", str(pathlib.Path(os.getenv("HOME"), ".kube/eksctl/clusters", os.getenv("EKS_CLUSTER_NAME", ""))))
...
def main(args = None):
    try:
        ...
        if os.path.isdir(config_path) is False:
            ...
            subprocess.check_call(f"git clone {git_tenant_url} {config_path}", shell=True)
