...
if not NEULABS_WORKSPACE:
    raise Exception("NEULABS_WORKSPACE not found in env. Configure neulabs env")

# WORKSPACE VARS
WS_CONFIGURATION_FILEPATH = pathlib.Path(NEULABS_PATH, ".projects.yml")
WS_VSCODE_EXTENSIONS_FILEPATH = pathlib.Path(NEULABS_WORKSPACE, ".vscode", "extensions.json")
WS_VSCODE_SETTINGS_FILEPATH = pathlib.Path(NEULABS_WORKSPACE, ".vscode", "settings.json")
WS_VSCODE_WORKSPACE_FILEPATH = pathlib.Path(NEULABS_WORKSPACE, "projects.code-workspace")
...
def main(args = None) -> bool:
    try:
        ...
        if args.filename != WS_CONFIGURATION_FILEPATH:
            file_parsed.update(**yaml.load(open(WS_CONFIGURATION_FILEPATH), Loader=yaml.FullLoader))

        file_parsed.update(**yaml.load(open(args.filename), Loader=yaml.FullLoader))
        ...
        for target in [ w for w in os.listdir(NEULABS_WORKSPACE) if w not in WS_EXCLUDE ]:
            existing_workspace[target] = os.listdir(pathlib.Path(NEULABS_WORKSPACE, target))
